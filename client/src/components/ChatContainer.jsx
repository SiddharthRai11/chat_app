...
const ChatContainer=() =>{
  ...
  },[selectedUser])//when selected user changes the messages are fetched
    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className="flex item-center gap-3 py-3 mx-4 border-b border-stone-500">
        ...
        <img onClick={()=>setSelectedUser(null)} src={assets.arrow_icon} alt=''
         className=' w-5 h-5 '/>
         {/*<img src={assets.help_icon} alt='' className='md:hidden h-4 w-4'/>*/}
         </div>
      </div>
      {/* ------chatArea ------*/}
          ...
          <div key={index} className={`flex items-end gap-2 justify-end
            ${msg.senderId !==authUser._id&& 'flex-row-reverse'}`}>
              ...
              {msg.image ? (
                ...
                break-all  text-white ${msg.senderId===authUser._id
                  ? 'bg-violet-500/30 text-white' : 'bg-green-500 text-white'
                }`}>
                  {msg.text}
                </p>
              )}
      ...
      <div className='absoulte bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        ...
        text-white placeholder-gray-400'></input>
