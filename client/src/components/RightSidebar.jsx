...
//Get all the images from the messages and store them in an array
useEffect(()=>{
    setMsgImages(
  ...
  return selectedUser && (
    ...
    ${selectedUser? 'max-md:hidden':''}`}>
      ...
      <hr className='border-[#ffffff50] my-4 mx-4'/>
      {/*----Media displayed----*/}
      <div className='px-5 text-xs'>
        <p>Media</p>
        <div className='mt-2 max-h-[250px] overflow-y-scroll grid grid-cols-3 gap-4 opacity-80'>
          {msgImages.map((url,index)=>(
            <div key={index} onClick={()=>window.open(url)} 
            className='cursor-pointer rounded'>
              <img src={url} alt='' className='w-full h-24 object-cover rounded-md shadow-md border border-gray-700'/>
            </div>
          ))}
